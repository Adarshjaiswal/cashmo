...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($user->user_type == 'user') {
            // Fetch data for a regular user
            $totalAmount = Wallet::where('user_id', $user->id)->get();
            $totalDeposits = WalletDeposite::where('user_id', $user->id)
                                           ->where('status', 'confirmed')
                                           ->sum('amount');
            $successfulRechargesCount = MobileRecharge::where('user_id', $user->id)
                                           ->where('status', 'success')
                                           ->count();
            $rechargeCommission = Commission::first();

            return view('dashboards.dashboard', compact('assets', 'totalAmount', 'totalDeposits', 'successfulRechargesCount', 'rechargeCommission'));
        } elseif ($user->user_type == 'admin') {
            // Fetch data for an admin
            $totalRetailers = User::where('user_type', 'user')->count();
            $totalWalletAmount = Wallet::sum('wallet_amount');
            $allRechargesCount = MobileRecharge::where('status', 'success')->count();
            $rechargeCommission = Commission::first();

            return view('dashboards.dashboard', compact('assets', 'totalRetailers', 'totalWalletAmount', 'allRechargesCount', 'rechargeCommission'));
        }
    }
